...
Id<HalfEdge> GradientMesh::extrude(Id<HalfEdge> edge)
{
  ...
  ::connect(twin, new_patch, edges, patches);
  ::connect(twin_next, new_patch, edges, patches);
  ::connect(twin_prev, new_patch, edges, patches);
  ::connect(twin_opp, new_patch, edges, patches);
