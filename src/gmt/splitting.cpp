...
#include "gmt/interval.hpp"
#include "gmt/snapping.hpp"
...
Id<HalfEdge> GradientMesh::split(Id<HalfEdge> top, float t)
{
  ...
  gmt::connect(top_left, left_patch, edges, patches);
  ...
  gmt::connect(top_right, right_patch, edges, patches);
                                 ...
                                 Id<HalfEdge> parent, Interval new_interval)
{
  ...
  for (auto it = begin; it != end; ++it)
  {
    ...
    f.interval = gmt::reparametrize(f.interval, new_interval);
    f.parent = parent;
    ...
    {
      next_f->interval = gmt::reparametrize(next_f->interval, new_interval);
